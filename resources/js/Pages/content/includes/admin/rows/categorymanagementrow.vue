<template>
    <tr>
        <td>
            <jet-input :id="'title' + item.id" type="text" :value="item.name" class="mt-1 block w-full"
                ref="description" />
        </td>
        <td>
        ...
        <td>
            <jet-input :id="'title' + item.id" type="text" :value="item.icon" class="mt-1 block w-full" ref="title" />
        </td>
        <td>
...
export default {
    ...
    methods: {
        handleEditAction() {
            this.$store.commit("StartSetEditCategoryState", this.item);
        },
        handleDeleteService() {
            this.$store.commit("StartDeleteCategory", this.item.id);
        }
    }
}
</script>
